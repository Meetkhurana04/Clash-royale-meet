...
// spawn hone ke baad bhi toh update krna chaiye kyuki us tiem km hua h 
 if (window.multi && !fromNetwork && window.socket && window.hasJoinedRoom) {
    // send normalized coordinates so different clients with different canvas sizes match
    const xPct = x / (canvas.width || 1);
    const yPct = y / (canvas.height || 1);
    window.socket.emit("spawn_character", {
      room: window.roomId || "",
      type,
      x,
      y,
      opp_or_not   // so receiver can invert it
    });
  }
