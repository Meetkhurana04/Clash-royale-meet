...
let activeplayers = []
let spawnSeq = 0;
// global seq for local ids


function makeActive(type, x, y, opp_or_not, fromNetwork = false, localId = null, serverId = null, owner_sid = null) {
  const chrKey = `chr_${type.toLowerCase()}_out`;
  const stats = attackpower[type.toLowerCase()];
  if (!stats) { console.error("Invalid type:", type); return; }
  let elixir_req = stats.elixir_needed;
  if (opp_or_not === 'opp') current_elixiropp -= elixir_req;
  if (opp_or_not === 'our') current_elixirour -= elixir_req;

  const nowIdLocal = localId || `${(window.socket && window.socket.id)||'anon'}_${Date.now()}_${(spawnSeq++)}`;

  const ent = {
    id: serverId || `${type}_${Date.now()}`,   // temporary id until server responds
    serverId: serverId || null,
    localId: localId || nowIdLocal,
    ...
    predicted: !fromNetwork // true = local predicted spawn
  };

  activeplayers.push(ent);
  konseactivehongeplayer(current_elixiropp, current_elixirour);

  // If this is a local spawn (not fromNetwork), and multiplayer configured, send request to server
  if (window.multi && !fromNetwork && window.socket && window.hasJoinedRoom) {
    const xPct = x / (canvas.width || 1);
    const yPct = y / (canvas.height || 1);
    const payload = {
      room: window.roomId || "",
      type,
      xPct,
    ...
    window.socket.emit("spawn_request", payload);
  }

  return ent;
}

// utility: find activeplayer by serverId or localId
function findActiveByLocalId(localId) {
  return activeplayers.find(p => p.localId && p.localId === localId);
}
function findActiveByServerId(serverId) {
  return activeplayers.find(p => p.serverId && p.serverId === serverId);
}

window.onServerSnapshot = function(snapshot) {
  if (!snapshot || !Array.isArray(snapshot.entities)) return;
  // build lookup of server entities
  const serverMap = {};
  for (const s of snapshot.entities) {
    serverMap[s.id] = s;
    // If entity has localId (echoed from spawner), attempt reconciliation
    if (s.localId) {
      const predicted = findActiveByLocalId(s.localId);
      if (predicted) {
        // attach server id & owner info
        predicted.serverId = s.id;
        predicted.owner_sid = s.owner_sid;
        // compute server pixel coords
        const serverX = Math.round((canvas.width || 1280) * s.xPct);
        const serverY = Math.round((canvas.height || 720) * s.yPct);
        // Smoothly correct position (lerp)
        predicted.x = predicted.x + (serverX - predicted.x) * 0.2;
        predicted.y = predicted.y + (serverY - predicted.y) * 0.2;
        // set health
        predicted.health = s.hp || predicted.health;
        predicted.predicted = false;
        continue; // handled
      }
    }

    // If not predicted by us, check if we already have server entity
    if (!findActiveByServerId(s.id)) {
      // Spawn a network-origin entity on this client
      const serverX = Math.round((canvas.width || 1280) * s.xPct);
      const serverY = Math.round((canvas.height || 720) * s.yPct);
      const mirroredOpp = (s.owner_sid && window.socket && s.owner_sid === window.socket.id) ? 'our' : 'opp';
      // If owner is other player, mirror xPct horizontally
      let spawnOpp = 'opp';
      if (s.owner_sid && window.socket && s.owner_sid === window.socket.id) spawnOpp = 'our';
      // Determine final opp flag relative to this client
      const finalOppFlag = (s.owner_sid && window.socket && s.owner_sid !== window.socket.id) ? 'opp' : 'our';

      // If entity came from other player, mirror X
      let xToUse = serverX;
      if (s.owner_sid && window.socket && s.owner_sid !== window.socket.id) {
        // mirror horizontally
        xToUse = canvas.width - serverX - (playersize / 2);
      }
      makeActive(s.type, xToUse, serverY, finalOppFlag, true, s.localId, s.id, s.owner_sid);
    } else {
      // existing server entity: update smoothly
      const local = findActiveByServerId(s.id);
      const serverX = Math.round((canvas.width || 1280) * s.xPct);
      const serverY = Math.round((canvas.height || 720) * s.yPct);
      local.x += (serverX - local.x) * 0.2;
      local.y += (serverY - local.y) * 0.2;
      local.health = s.hp || local.health;
    }
  }

  // Cleanup: remove any local entity with serverId not present in serverMap
  const toRemove = [];
  for (const p of activeplayers) {
    if (p.serverId && !serverMap[p.serverId]) {
      toRemove.push(p);
    }
  }
  if (toRemove.length) {
    activeplayers = activeplayers.filter(p => !toRemove.includes(p));
  }

  // Optionally update global timer if provided
  if (snapshot.time_left != null) {
    absolutecounter = snapshot.time_left;
  }
};
