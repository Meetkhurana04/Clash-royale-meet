...
rooms_state = {}   # room -> { entities: {serverId:entity}, input_queue: [requests], tick: int, start_time: float, time_left: float }

def ensure_room_state(room):
    if room not in rooms_state:
        rooms_state[room] = {"entities": {}, "input_queue": [], "tick": 0, "start_time": None, "time_left": None}
    return rooms_state[room]
...
def home():
    ...
    is_private_flag = request.form.get("is_private", "")  # optional from form (checkbox)
